...
const ChatApp = () => {
    const { currentUser }= useApp();
    const [Users, setUsers] = useState<string[]>([]);
    const [selectedUser, setSelectedUser] = useState<string | null>('Select user for chats'); // Initial value set to prompt the user
    const [messages, setMessages] = useState<Msg[]>([]);
    const [content, setContent] = useState<string>("");
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            try {
                if (!selectedUser || selectedUser === 'Select user for chats') return; // Don't fetch messages if no user is selected or the prompt is selected
                const db = getDatabase(app);
                const messagesRef = ref(db, `Users/${selectedUser}/msgs`);
                onValue(messagesRef, (snapshot: DataSnapshot) => {
                    const data = snapshot.val();
